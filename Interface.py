...
def btn_parametrs():  # окно для параметров
    def vvod():
        ...
        maxdep = win_1.get()  # ввод значений
        mindep = win_2.get()
        xyxy = win_3.get()
...
class Menu(Frame):  # меню
    ...
    def Start(self, num):  # вызов функций Димы и Вани
        if num == 1:
            from TransformMatrix import Ivan_func
            Ivan_func(filename, maxdep, mindep)

            #from Graph import Dima_func
            #Dima_func(filename)

            from RoutePlanning import Yura_func
            Yura_func(filename,coordinates)
